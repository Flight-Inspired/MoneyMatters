...
def get_top_donors(bioguide_id):
    try:
        ...
        if result:
            ...
            # Now fetch the top 10 donors for the retrieved cid, sorted by total amount
            c.execute('''
            ...
            # Sort the top donors based on their numeric values before formatting
            sorted_top_donors = sorted(top_donors, key=lambda x: int(x[1]), reverse=True)

            # Format the donor amounts with commas
